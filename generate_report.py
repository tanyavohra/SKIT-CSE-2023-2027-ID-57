...
def create_charts(students, timeline_activity, interval):
    ...
    if periods and authors:
        for author in authors:
            counts = [
                timeline_activity[period].get(author, 0)
    ...
    if authors:
        net_loc = [
            students[author]["added"] - students[author]["deleted"]
            for author in authors
        ]
...
def generate_pdf(interval="weekly"):
    ...
    students, timeline_activity, student_logs, scope_title = result
    ...
    if interval == "weekly":
        ...
        doc_name = (
            f"{repo_name}_Weekly_Progress_Report_Form-3_{date_stamp}.pdf"
        )

    elif interval == "monthly":
        report_title = "Monthly Progress Report (Form-3)"
        doc_name = (
            f"{repo_name}_Monthly_Progress_Report_Form-3_{date_stamp}.pdf"
        )
    ...
    total_commits = sum(
        data["commits"] for data in students.values()
    )
    ...
    if not student_logs:
        ...
        for student_name, logs in student_logs.items():
            ...
            student_section.append(
                Paragraph(
                    f"<b>Student:</b> {html.escape(student_name)} — "
            ...
            log_table_data.append(
                [
                    ...
                    Paragraph(
                        f"<font name='Courier'>{html.escape(first_sha)}</font>",
                        meta_cell_style,
                    ),
            ...
            for date_val, sha_val, msg_val in logs[1:]:
                ...
                log_table_data.append(
                    [
                        ...
                        Paragraph(
                            f"<font name='Courier'>{html.escape(sha_val)}</font>",
                            meta_cell_style,
                        ),
                        Paragraph(safe_msg, msg_style),
                        "",
    ...
    print(
        f" -> Found {len(students)} student(s) and "
...
if __name__ == "__main__":
    ...
    if chosen_interval not in {"weekly", "monthly", "final"}:
        print(
            "[ERROR] Invalid interval. Use: weekly, monthly, or final."
        )
        sys.exit(1)
